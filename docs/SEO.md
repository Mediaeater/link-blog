...
### ItemList Schema (Build-time Generated)
```json
{
  "@type": "ItemList",
...
**Generated by:** `scripts/generate-itemlist.js`
**Benefits:**
- Rich search results with individual link entries
...
3. **Build-time prerendering** - `<noscript>` block with 100 most recent links as semantic HTML (articles, blockquotes, time elements) so crawlers see real content
4. **Structured data** - WebSite, CollectionPage, and ItemList JSON-LD schemas
5. **ItemList JSON-LD** - 50 most recent links as Schema.org ListItem > Article entries, injected at build time
6. **Cache headers** - Dev server serves feeds with Cache-Control, ETag headers; `public/_headers` for CDN platforms
...
*Last updated: 2026-03-05 by Claude Code*
