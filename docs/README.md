# Documentation Index

Welcome to the Link Blog documentation!

## Getting Started

- [Quick Start](setup/quickstart.md) - Get up and running in 5 minutes

## Features

- [ActivityPub & Fediverse](features/activitypub.md) - Federated social features
- [Feeds & Syndication](features/feeds.md) - Atom, JSON Feed, OPML, Sitemap
- [SEO Optimization](SEO.md) - Search engine optimization features

## Reference

- [Scripts Reference](reference/scripts.md) - All available npm scripts and utilities
- [NPM Commands Cheat Sheet](NPM-COMMANDS.md) - Quick command reference
- [Sync Workflow](SYNC-WORKFLOW.md) - Newsfeeds.net sync workflow
- [Archive System](ARCHIVE_SYSTEM.md) - Link archiving by year
- [Deployment Checklist](DEPLOYMENT_CHECKLIST.md) - SEO deployment steps

## Workflow

- [Development Workflow](WORKFLOW.md) - Git workflow, testing, and best practices

---

**Quick Links:**
- [Main README](../README.md)
- [Project State (CLAUDE.md)](../CLAUDE.md)
- [Pickup Notes (PICKUP.md)](../PICKUP.md)
